...
class PatrollerSprite(prefab_sprites.MazeWalker):
  """A `Sprite` for our patroller, walking from top to bottom and back."""
  ...
  def update(self, actions, board, layers, backdrop, things, the_plot):
    ...
    if layers['#'][row-1, col]: self._moving_north = False
    if layers['#'][row+1, col]: self._moving_north = True

    # Make our move. If we're now in the same cell as the player, it's instant
    # game over!
    (self._north if self._moving_north else self._south)(board, the_plot)
...
class PlayerSprite(prefab_sprites.MazeWalker):
  ...
  def update(self, actions, board, layers, backdrop, things, the_plot):
    ...
    elif actions == 3:  # go up
      self._west(board, the_plot)
    elif actions == 4:  # go down
...
class PoisonDrape(plab_things.Drape):
  ...
  This Drape detects when a player traverses a poison, removing the poison and
  crediting the player for the collection. Terminates if all coins are gone.
  """
...
class DeadlyGridEnv(gym.Env):
    ...
    def _to_obs(self, observation):
        ...
        ob = np.stack([observation.layers[c] for c in 'Pa$@'] + [hallway], axis=2).astype(np.uint8)
        return ob
